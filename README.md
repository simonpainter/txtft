# txtft — TXT file transfer over DNS
...
| Tool | Role |
|------|------|
| `encode` | Zip → segment → base64 → write a zone-store JSON, print the seed key. |
| `serve` | Minimal authoritative `TXT` responder (asyncio UDP + TCP) over that store. |
| `fetch` | Resolve the manifest, walk the index chain, fetch/decode/reassemble/verify. |

## Install

```bash
pip install -r requirements.txt          # just dnspython
# optional, for console commands:
pip install -e .
```

Python 3.11+. Either invoke as modules (`python -m encode ...`) or, after
`pip install -e .`, use the packaged CLI commands (`txtfs-encode` / `txtfs-serve` / `txtfs-fetch`).
...
**dedicated subdomain** to the box running `serve`, so you never touch the apex
zone. At the parent zone (`example.com`):

```
f.example.com.   IN  NS   ns-txtft.example.com.
ns-txtft.example.com. IN A   203.0.113.10
```

Then bind the server to `:53` (needs privilege) and point `fetch` at the zone
...
Three independent layers, all checked by `fetch`:
...
├── encode.py       # encode CLI/module
├── serve.py        # serve CLI/module (asyncio UDP + TCP)
├── fetch.py        # fetch CLI/module
